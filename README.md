...
- 🎉📅 **報名日期偵測**：偵測 `C{N}Appset.html` 出現且成功解析出報名日期，依「今天是否已到報名開始日」分別發送 🎉「報名開始」或 📅「報名日期公告（尚未開始受理）」通知
...
| `appset` | `C{N}Appset.html` 出現 HTTP 200 **且**成功解析出報名日期 | 報名受付開始（含日期） |

每次執行時**優先檢查 Appset 頁**，若已開放則略過 Schedule 頁直接進入 appset 判斷。

⚠️ Appset 頁有時會先以「頁面已建立、日期尚未公告」的形式出現（HTTP 200 但頁面內無日期文字）。這種情況下腳本**不會**升級為 `appset` 狀態、也不會發送通知，會維持原狀態並在隔天重新嘗試解析，直到真的抓到日期為止。
...
| `pending`/`schedule` → `appset`，**已解析出日期且已到報名開始日** | 🎉 紫色大通知（非靜音） |
| `pending`/`schedule` → `appset`，**已解析出日期但還沒到報名開始日** | 📅 藍色通知（非靜音，標題為「報名日期公告！」） |
| Appset 頁已建立但尚未解析到日期 | 不發通知，隔天自動重試 |
| 主頁解析失敗（可能格式變更） | ⚠️ 紅色警告（非靜音） |
| 每週一腳本執行 | 📋 灰色靜音確認 |
| 首次執行偵測到屆次 | 靜默記錄，不發通知 |

每週靜音確認依狀態分四個區塊：
- **報名進行中**（截止日未到；截止日以 **粗體＋⚠️** 醒目提醒即將截止）
- **報名截止・活動待舉辦**（截止日已過但活動未舉辦；截止日恢復一般文字，並標註「（報名已截止）」）
- **日程已公告**（schedule 狀態）
- **尚未公告**（pending 狀態）
...
- 若當天是週一，Discord 頻道會額外收到「📋 Comiket・每週確認」靜音訊息；非週一執行則不會有這則訊息（僅在偵測到狀態變化時才會發送對應通知）
